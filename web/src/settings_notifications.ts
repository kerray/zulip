...
// has no subscription and has not yet granted notification permission. Its
// button requests permission and subscribes inside the click's user gesture.
...
// but this browser still needs to be enrolled (no local subscription and
// permission not yet granted — the auto-enroll on load only covers the
// already-granted case). Otherwise removes any stale banner.
