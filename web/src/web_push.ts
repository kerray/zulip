...
// Notification permission requires a user gesture, so a browser is enrolled
// only from an explicit user action taken in it (handle_setting_change, from
// the settings toggle or the per-browser enrollment banner). initialize() only
// MAINTAINS an enrollment this browser already has: it re-posts the existing
// subscription to keep the server's copy fresh, and recreates it against the
// current VAPID key if the server's key was rotated. If the setting is off but
// a stale subscription lingers (e.g. from a previous account holder in this
// browser), initialize() destroys it.
...
async function run_after<T>(previous: Promise<void>, op: () => Promise<T>): Promise<T> {
    await previous;
    return await op();
}

// Advance the chain regardless of an op's outcome, swallowing its rejection on
// the shared tail so one failed op neither breaks the chain nor surfaces as an
// unhandled rejection. Callers still observe the rejection via their own handle
// on the op's promise.
async function settle(result: Promise<unknown>): Promise<void> {
    try {
        await result;
    } catch {
...
// This is deliberately not an async function: it must fork the
// failure-swallowing tail from the caller-visible promise synchronously, WITHOUT
// awaiting either, so that the next enqueue (which may happen before this op
// even starts) chains behind this one. Awaiting here would serialize the fork
// away and change the queue semantics.
// eslint-disable-next-line @typescript-eslint/promise-function-async
function enqueue<T>(op: () => Promise<T>): Promise<T> {
    const result = run_after(pending, op);
...
// A VAPID key rotation leaves an existing subscription bound to the old key,
// which the server can no longer send to. The push service rejects those sends
// with 403, not the 404/410 that prunes a subscription server-side, so a
// browser left on a stale key stays silently undeliverable until it
// re-subscribes; both the subscribe and the load-time sync paths therefore
// check the key they already hold.
function subscription_uses_current_key(subscription: PushSubscription): boolean {
    const application_server_key = subscription.options.applicationServerKey;
    if (application_server_key === null) {
        return false;
    }
    return array_buffers_equal(
        application_server_key,
        base64url_to_uint8_array(realm.server_web_push_vapid_public_key),
    );
}
...
async function subscribe(): Promise<void> {
    ...
    // Drop a subscription bound to a since-rotated key, so we re-subscribe with
    // the current one rather than stranding this browser.
    if (existing !== null && !subscription_uses_current_key(existing)) {
...
// Runs on page load. It maintains a subscription this browser already holds —
// re-posting it, or recreating it against a rotated VAPID key — but it never
// enrolls a browser that holds none, and so never triggers a permission
// prompt. Enrolling a browser happens only from handle_setting_change, inside
// a user gesture in that browser.
async function sync_on_load(my_generation: number): Promise<void> {
    ...
    if (user_settings.enable_web_push_notifications) {
        if (subscription === null) {
            // The setting is on but this browser is not enrolled. Do not
            // enroll it here: the setting is account-wide while subscriptions
            // are per-browser, so it says nothing about whether the user wants
            // notifications on THIS browser, and a granted Notification
            // permission is not evidence of that either — Zulip also asks for
            // it for desktop notifications. The notification settings surface
            // a banner offering to enroll this browser explicitly.
            return;
        }
        if (subscription_uses_current_key(subscription)) {
            ...
            // The server's VAPID key was rotated since this browser
            // subscribed, so re-posting would just refresh a row the push
            // service will reject. Recreate the subscription against the
            // current key: this browser is already enrolled, so this preserves
            // an existing opt-in rather than creating a new one, and with the
            // permission already granted subscribe() cannot prompt.
            await subscribe();
        }
        return;
    }
    ...
    if (subscription !== null) {
        ...
        await subscription.unsubscribe();
        try {
            await delete_subscription_on_server(endpoint);
        } catch (error: unknown) {
