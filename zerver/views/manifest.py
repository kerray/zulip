...
def web_app_manifest_icon(request: HttpRequest, size: int) -> HttpResponse:
    ...
    cached hard -- but only at the realm's *current* icon version. Serving
    today's bytes with an immutable header at a not-yet-current (or
    already-superseded) version URL would let a requester poison shared
    caches with stale icon bytes under that URL for a year, e.g. by
    requesting `?version=<current + 1>` before the realm's icon actually
    ...
    if request.GET.get("version") != str(realm.icon_version):
        # Redirect to the canonical, current-version URL instead of serving
        # (and immutably caching) the current icon under a mismatched
        # version URL. This response deliberately carries no long-lived
        # Cache-Control: once the realm's icon changes again, this same URL
        # needs to redirect somewhere else.
        # Built only from the validated size and the server-side icon
        # version — never from request data — so the target cannot be
        # steered by the requester.
        canonical_url = f"/manifest/icon-{size}.png?version={realm.icon_version}"
