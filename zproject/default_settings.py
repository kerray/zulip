...
#
# Off by default: generate-secrets seeds a VAPID keypair on every server,
# including on upgrade, so gating on the secret alone would silently turn
# the feature on for every existing installation. A server operator opts in
# by setting this in /etc/zulip/settings.py.
